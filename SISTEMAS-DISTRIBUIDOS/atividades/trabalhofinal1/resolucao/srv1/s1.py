    ...
    while 1:
        ...
        if (data.decode() == "file"):
            try:
                ...
                # abre o rel1.txt que possui a nota do par para downloads de arquivos 
                with open('rel1.txt', 'r') as key_file:
                    result = key_file.read()

                # concatenação de strings para retornar ao par externo que possui o arquivo para download
                conctResult = "file disponivel from 225.0.0.1 id=1 " + result
                ...
                # se o primeiro termo da lista for 1 ele continuar a execução para enviar o arquivo linha-a-linha
                if (r[0] == "1"):
                ...
                print(e)
                serverSocket.sendto(
                    "Arquivo inexistente from 225.0.0.1 ou Falha na conexao".
                    encode(), ('', addr[1]))
